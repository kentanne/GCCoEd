<template>
		...
		<div class="lower-grid">
		  <div class="personal-information">
			...
			<div class="input-wrapper">
			  <div v-for="(item, index) in inputFieldPersonalInformation" :key="index" class="input-fields">
				...
				<!-- Select Dropdown -->
				<div v-else-if="item.type === 'select' && item.field !== 'Gender'" class="custom-dropdown">
				  <div class="dropdown-container" @click.stop="toggleDropdown(toCamelCase(item.field))">
					<input
					  type="text"
					  :value="personalData[toCamelCase(item.field)]"
					  :placeholder="personalData[toCamelCase(item.field)] || `Select ${item.field.toLowerCase()}`"
					  readonly
					  class="standard-input"
					/>
					<i class="dropdown-icon" :class="{ 'open': dropdownOpen[toCamelCase(item.field)] }">▼</i>
				  </div>
				  <div v-if="dropdownOpen[toCamelCase(item.field)]" class="dropdown-options">
					<div
					  v-for="(option, i) in item.options"
					  :key="i"
					  class="dropdown-option"
					  @click="selectOption(toCamelCase(item.field), option, 'personal')"
					>
					  {{ option }}
					</div>
				  </div>
				...
				<div v-else-if="item.field === 'Gender'" class="gender-section">
				  <div class="gender-dropdown">
					<div class="dropdown-container" @click.stop="toggleDropdown('gender')">
					  <input
						type="text"
						...
						readonly
					  />
					  <i class="dropdown-icon" :class="{ 'open': dropdownOpen.gender }">▼</i>
					</div>
					<div v-show="dropdownOpen.gender" class="dropdown-options gender-options">
				  ...
				  <!-- Other Gender Input -->
				  <div v-if="personalData.gender === 'Other'" class="other-gender-input">
		  ...
		  <div class="profile-information">
			...
			<div class="input-wrapper">
			  <div v-for="(item, index) in inputFieldProfileInformation" :key="index" class="input-fields">
				<label>{{ item.field }}</label>
				...
				<!-- Select Dropdown -->
				<div v-else-if="item.type === 'select' && item.field !== 'Course Offered'" class="custom-dropdown">
				  <div class="dropdown-container" @click.stop="toggleDropdown(toCamelCase(item.field))">
					<input
					  type="text"
					  :value="profileData[toCamelCase(item.field)]"
					  :placeholder="profileData[toCamelCase(item.field)] || `Select ${item.field.toLowerCase()}`"
					  readonly
					  class="standard-input"
					/>
					<i class="dropdown-icon" :class="{ 'open': dropdownOpen[toCamelCase(item.field)] }">▼</i>
				...
				<div v-else-if="item.field === 'Course Offered'" class="custom-dropdown">
				  <div class="dropdown-container" @click.stop="toggleDropdown(toCamelCase(item.field))">
					<input
					  type="text"
					  :value="getCourseOfferedDisplay()"
					  :placeholder="getCourseOfferedDisplay() || 'Select courses'"
					  readonly
					  class="standard-input"
					/>
					<i class="dropdown-icon" :class="{ 'open': dropdownOpen[toCamelCase(item.field)] }">▼</i>
				  </div>
				  <div v-if="dropdownOpen[toCamelCase(item.field)]" class="dropdown-options checkbox-options">
					<div class="category-section" v-if="availableSubjects.coreSubjects.length > 0">
					  <h4>Core Subjects</h4>
					  <div
					...
					<div class="category-section" v-if="availableSubjects.gecSubjects.length > 0">
					  <h4>GEC Subjects</h4>
					  <div
					...
					<div class="category-section" v-if="availableSubjects.peNstpSubjects.length > 0">
				  ...
				  <div v-if="dropdownOpen[toCamelCase(item.field)]" class="dropdown-options checkbox-options">
					...
					>
					  <input
						...
						@click.stop
					  />
					  <label :for="`${toCamelCase(item.field)}-${i}`">{{ option.label }}</label>
					</div>
				  </div>
				</div>
			  </div>
		  ...
		  <div class="bio-experience-grid">
			...
			>
			  ...
			  <textarea
				...
				:placeholder="item.field === 'Short Bio' ? 'Tell us about yourself' : 'Describe your tutoring experience'"
			  ></textarea>
			</div>
		  </div>
		</div>
  ...
  watch(() => personalData.program, (newProgram) => {
	...
	switch(program) {
	  case "Bachelor of Science in Information Technology (BSIT)":
		availableSubjects.coreSubjects = [
	          "Application Development and Emerging Technologies", "Business Analytics", "Computer Programming 1", "Computer Programming 2", 
	  ...
	  case "Bachelor of Science in Computer Science (BSCS)":
		availableSubjects.coreSubjects = [
              "Computer Programming 1", "Computer Programming 2",  "Introduction to Computing", 
	  ...
	  case "Bachelor of Science in Entertainment and Multimedia Computing (BSEMC)":
		availableSubjects.coreSubjects = [
            "Introduction to EM Computing", "Computer Programming 1", "PC Troubleshooting with Basic Electronics", "Computer Programming 2", 
  ...
  const getDisplayValue = (field) => {
	if (Array.isArray(profileData[field])) {
	  return profileData[field].join(', ') || `Select ${field.replace(/([A-Z])/g, ' $1').toLowerCase()}`;
	}
	return profileData[field] || `Select ${field.replace(/([A-Z])/g, ' $1').toLowerCase()}`;
  };
  ...
  const saveChanges = () => {
	console.log("Changes saved:", { personalData, profileData });
	emit("close");
  };
  </script>
  ...
  .bio-experience-grid .column-1 {
	grid-column: 1;
  }
  
  .bio-experience-grid .column-2 {
	grid-column: 2;
  }
